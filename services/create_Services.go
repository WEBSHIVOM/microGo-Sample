...
import (
	github2 "github.com/federicoleon/golang-microservices/src/api/domain/github"
	"microGo/src/api/config"
	"microGo/src/api/domain/github"
	"microGo/src/api/domain/repositories"
	"microGo/src/api/log/option_b"
	"microGo/src/api/providers/github_provider"
...
func (s *reposService) CreateRepo(clientId string, input repositories.CreateRepoRequest) (*repositories.CreateRepoResponse, errors.ApiError) {
	...
	response, err := github_provider.CreateRepo(config.GetGithubAccessToken(), github2.CreateRepoRequest(request))
	if err != nil {
